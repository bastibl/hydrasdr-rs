...
This note records the public Rust API layered on top of the verified C-to-Rust translation. The direct layer remains inside the crate as the USB/control implementation and parity test bed; downstream users should work through the ergonomic API.

## Inputs from the direct translation

The implementation has an internal direct layer that mirrors the C driver:

- `HydraSdr` is the internal device handle with sync and async control methods.
- `discovery::{list_devices, list_devices_async}` expose visible RFOne descriptors internally.
- `types::{DeviceInfo, SampleType, GainInfo, ...}` and `commands::{GainType, RfPort, ...}` mirror C enums/structs where those values are still needed.
- `streaming::{Transfer, StreamingStats}` expose the internal callback contract: raw USB bytes, C-parity sample counts, and non-zero callback return for stop. `DirectRxStream` provides pull-style unpacked `Float32Iq` conversion with C-style DDC decimation.
...
The public crate root exposes ergonomic types only:
...
hydrasdr_rs::RfPort             // public RF input selector
hydrasdr_rs::GainType           // public low-level gain selector where needed by config
hydrasdr_rs::StreamingStats     // receive counters
```

The direct modules are private. Internal tests may still construct fake control/streaming backends and `HydraSdr` handles, but downstream crates should not be able to depend on C-shaped request builders, callback transfer structs, or direct handle methods.
...
Builder methods return `Self` for configuration and `Result<Device>` only at `open()`/`open_async()`. `Device::configure(&Config)` and `Device::configure_async(&Config)` reconfigure an already-open device; direct `Config::apply_direct` helpers stay crate-private.

## `Device` ownership model

`Device` owns one internal `HydraSdr<NusbControl>` and a cached `DeviceInfo` after open/configuration. It should not be `Clone`. Keep mutable access for control operations so streaming and configuration cannot race in safe code.
...
- `stop()` is idempotent and sends receiver off through the internal control layer.
...
- `unstable-direct-aliases`: not needed; keep direct aliases private unless there is a specific, reviewed compatibility requirement.

Do not add a dependency on a specific async runtime beyond the existing `nusb` feature forwarding.

## Implementation plan

1. Keep the C-port modules private and use internal tests for parity/fake-backend coverage.
2. Expose `Config`, `Device`, `DeviceBuilder`, `RxStream`, `AsyncRxStream`, and `SampleBlock` from the crate root.
3. Keep `Config::apply_direct`, fake backend hooks, and direct stream primitives crate-private.
4. Keep examples focused on `rx_sync.rs` and `rx_async.rs`.
5. Run the existing verification matrix (`fmt`, clippy all targets/features, tests, examples, docs) before committing each implementation slice.
